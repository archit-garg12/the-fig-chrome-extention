...
function stemTags(tags, stemmedSource, callback) {
  ...
  for (let i = 0; i < stemmedSource.length; i++) {
    ...
    if (keywordToTagSplitBySpace[lowerTag] != undefined) {
		...
		{
			tagFrequency[lowerTag] += 1/keywordToTagSplitBySpace[lowerTag].length;
  ...
  if(sortedTagFreq[0][0].length == 1)
  {
	  tagsToSend = stemToReal[JSON.stringify(sortedTagFreq[0][0][0])]
	  
  }else
  {
	for(let i = 0; i < sortedTagFreq[0][0].length;i++)
	{
		allTagsScores = 0;
		for( let k ; k < sortedTagFreq[0][0][i].length; k++)
		{
			allTagsScores += tagFrequency[sortedTagFreq[0][0][i][k]];
		}
	...
	tagsToSend = stemToReal[JSON.stringify(sortedTagFreq[0][0][currentTagMaxIndex])]
